...
const Input = React.forwardRef<HTMLInputElement, InputProps>(
  (
    ...
    const preventMousedownTriggerBlur = (
    ...
    const elipsisTextboxWidthStyle = {
      maxWidth: !!dropdownIcon ? "calc(100% - 2.75rem)" : "100%",
    };
    const inputMaskClassname = classnames(
      " pt-6 body2 h-14 flex-shrink flex-grow flex-auto leading-normal w-px flex-1 input-date-mask",
      {
        "pl-7": !!props.prefix,
        "pl-4": !props.prefix,
      }
    ...
    let component =
      props.applyTruncating && props.disabled ? (
        <div className="flex pt-6 pl-4 h-14" style={elipsisTextboxWidthStyle}>
    ...
    return (
      <div
        className={classnames(
          "flex flex-col h-20 input-group",
          containerClassName
        )}
      >
        <div
          className={
            `input-group-container flex flex-wrap items-stretch w-full relative ${
        ...
        >
          ...
          <label
            htmlFor={htmlFor}
            className={`absolute truncate ${props.required ? "required" : ""} ${
              isFocused ||
              props.value ||
              (!!props?.value?.toString() && props.value.toString().length) > 0
                ? "body3 label-small"
                : `body2${props.disabled ? "-medium" : ""}`
          ...
          {dropdownIcon && (!isFocused || !props.value) && (
            <div
              className={`absolute pt-${!label ? "3" : "4"} right-4 ${
                dropdownIconClickHandler ? "cursor-pointer" : ""
              }`}
            >
              {
                <SvgIcon
                  ...
                  fillClass={dropdownIconFill ?? "select-arrow-fill"}
                />
              }
            </div>
          )}
          ...
          {autoSuggestDropdown && isFocused && (
            <div
              className={`input-options ${
                dropdownOpen ? "options-visible" : "hidden"
              } absolute block py-2`}
            >
        ...
        {!!assistiveText && !props.error && !isLoading && (
          <div
            className={`h-6 pl-4 body3 pt-1 truncate ${
              isFocused ? "assistive-text-focus" : ""
            }`}
          >
            {t(assistiveText)}
          </div>
        ...
        {props.error && (
          <div className={"h6 pl-4 body3 pt-1 text-danger"}>
            {t(props.error)}
          </div>
        )}
      </div>
