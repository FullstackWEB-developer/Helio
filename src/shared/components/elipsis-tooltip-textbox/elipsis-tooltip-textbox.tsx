...
const ElipsisTooltipTextbox = ({
  value,
  asSpan = false,
  classNames,
  yOffsetInPixels,
  isDefaultTextClass = true,
  hasInlineBlock = true,
  tooltipClass = '',
}: ElipsisTooltipTextboxProps) => {
  ...
  return (
    <div ref={ref} className={classnames('truncate', { 'relative w-full h-full': isDefaultTextClass, 'inline-block': hasInlineBlock })}>
      {asSpan ? (
        <span onMouseEnter={() => setIsVisible(true)} onMouseLeave={() => setIsVisible(false)} className={classes}>
          {t(value)}
        </span>
      ) : (
        <div onMouseEnter={() => setIsVisible(true)} onMouseLeave={() => setIsVisible(false)} className={classes}>
          {t(value)}
        </div>
      )}
