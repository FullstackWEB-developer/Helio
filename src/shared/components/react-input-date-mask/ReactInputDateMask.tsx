...
const ReactInputDateMask = ({
    ...
    const [letterObject, setLetterObject] = useState<any>({})
    ...
    useEffect(() => {
        ...
        myRef?.current?.setSelectionRange?.(Number(moveCursor.start), Number(moveCursor.end))
    ...
    const findDigitsOrLettersInValue = ({ value, looking }: {value: any; looking: string}) => {
        const separator = value['3']
        const regex = {
            digits: /[0-9]/g,
            letters: /[mMyYdD]/
        } as any;
        const resultArray = Object.values(value).filter(el => el !== separator).map((el: any) => el.search(regex[looking])).filter(el => el === 0);
        return resultArray.length;
    }

    const isCurrValueHaveDigits = (currValue: any) => {
    ...
    const onTouchStart = (e: any): any => {
        trackingCursorPos(e)
    }

    const checkOneValue = (val: any, valueString: any, position: any) => {
        const regex = {
    ...
    const deletingElement = ({ pos, currentValue }: any) => {
    ...
    const onHandlePaste = async ({ target: { selectionStart }, clipboardData }: any) => {
        const pasteRaw = clipboardData?.getData('text') || await navigator.clipboard.readText();
