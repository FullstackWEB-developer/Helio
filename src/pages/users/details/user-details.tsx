...
import {GetUserConnect, GetUserExtended} from "@constants/react-query-constants";
...
const UserDetails = () => {
    ...
    const loadUserData = (data: UserDetailExtended) => {
        ...
        if (user.callForwardingValue) {
            setValue('forward_to_value', user.callForwardingValue);
        }
    ...
    const changeUserStatusMutation = useMutation(
        (param: {userId: string, userStatus: UserDetailStatus}) => changeUserStatus(param.userId, param.userStatus),
        {
            onSuccess: (data) => {
    ...
    const changeStatus = () => {
        ...
        const newStatus = user.status === UserDetailStatus.Active ? UserDetailStatus.Inactive : UserDetailStatus.Active;
    ...
    const saveUser = (formData: any) => {
        ...
        user.callForwardingType = formData.forward_to ? Number(formData.forward_to) as CallForwardingType : undefined;
        user.callForwardingValue = formData.forward_to_value;
    ...
    return (
        <div className='flex flex-row w-full px-6 py-8 user-details'>
            <div className='flex flex-col items-center pr-6 user-details-sidebar'>
                <Avatar
                    className='w-40 h-40'
                ...
                <div className='flex flex-col pt-5'>
                    <div className='flex flex-col'>
                        ...
                        <div className='flex flex-col mt-5'>
                            <span className='body2-medium'>{t('users.info_section.email')}<span className='body2 pl-0.5'>{userDetailExtended?.user.email}</span></span>
                            <span className='body2-medium'>{t('users.info_section.department')}<span className='body2 pl-0.5'>{userDetailExtended?.user.department}</span></span>
                        </div>
                    </div>
                    ...
                    <div className='flex flex-row mt-8 pr-28'>
                        <div className='flex-1 pr-4'>
                            <div className='flex flex-row items-center justify-between pr-7'>
                                ...
                                <a rel='noreferrer' target='_blank' className="body2" href={userDetailExtended?.contactProfileLink}>{t('common.change')}</a>
                            </div>
                        ...
                        <div className='flex-1'>
                            ...
                            {forwardToSelected === CallForwardingType.Phone &&
                                <ControlledInput
                                    name="forward_to_value"
                                    type="tel"
                                    control={control}
                                    label={t('users.call_forwarding_value_phone')}
                                />
                            }
                            ...
                            {forwardToSelected === CallForwardingType.Agent &&
                                <ControlledSelect
                                    control={control}
                                    name='forward_to_value'
                                    label={t('users.call_forwarding_value_agent')}
                                    defaultValue=''
