...
const GetMessageContent = (message: ChatMessage) => {
    ...
    if (message.Attachments) {
        return `File uploaded: ${message.Attachments[0]?.AttachmentName}`
    }
}
...
const printTranscriptMessage = (transcript: ChatMessage, nextTranscript?: ChatMessage) => {
    ...
    if ((transcript.Type === MessageTypeMessage && (transcript.ContentType === ContentTypePlainText || transcript.ContentType === ContentTypeInteractive)) || transcript.Type === MessageTypeAttachment) {
        ...
        if (nextTranscript && ((nextTranscript.Type === MessageTypeMessage && (nextTranscript.ContentType === ContentTypePlainText || nextTranscript.ContentType === ContentTypeInteractive)) ||
            nextTranscript.Type === MessageTypeAttachment) && isNextMessageSamePersonAndTime(transcript.DisplayName ?? '', transcript.AbsoluteTime, nextTranscript)) {
            output += newLine;
