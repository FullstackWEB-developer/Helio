...
const TicketNew = () => {
  ...
  const sourceOptions = useSelector(state =>
    selectEnumValuesAsOptions(state, 'TicketChannel'),
  );
  ...
  const ticketLookupValuesReason = useSelector(state =>
    selectLookupValues(state, 'TicketReason'),
  );
  const ticketLookupValuesSubject = useSelector(state =>
    selectLookupValues(state, 'TicketSubject'),
  );
  const tagOptions = useSelector(state =>
    selectLookupValuesAsOptions(state, 'TicketTags'),
  );
  ...
  const isTicketEnumValuesLoading = useSelector(
    selectIsTicketEnumValuesLoading,
  );
  ...
  const [debounceContactSearchTerm] = useDebounce(
    contactSearchTerm,
  ...
  const patientExcludedTicketTypes = [
    TicketType.BusinessOffice,
    TicketType.Facility,
    TicketType.Lab,
    TicketType.Pharmacy,
  ];
  const contactExcludedTicketTypes = [
    TicketType.EstablishedPatient,
    TicketType.NewPatient,
  ];
  const renderTicketTypeOptions = useCallback(() => {
    if (queryPatientId) {
      return ticketTypeOptions?.filter(
        o => !patientExcludedTicketTypes.includes(Number(o.value)),
      );
    }
    if (queryContactId) {
      return ticketTypeOptions?.filter(
        o => !contactExcludedTicketTypes.includes(Number(o.value)),
      );
    }
  ...
  const {
    ...
    [QueryContacts, debounceContactSearchTerm],
    () => {
    ...
    },
    {
      enabled: false,
      onSuccess: data => {
        const contactOptionResult = data.map(
          item =>
            ({
              label:
                item.type === ContactType.Company
                  ? item.companyName
      ...
      onError: () => {
        setError('contactId', {
          type: 'notFound',
          message: t('ticket_new.error_getting_contacts'),
        });
      },
    },
  );
    ...
    onSuccess: data => {
      dispatch(
        addSnackbarMessage({
          ...
          message: t('tickets.ticket_created', {
  ...
  const { data: contact, refetch: refetchContact } = useQuery<
    Contact,
    AxiosError
  >([GetContactById, queryContactId], () => getContactById(queryContactId!), {
    enabled: false,
    onSuccess: data => {
      if (data) {
  ...
  const onSubmit = async (formData: any) => {
    ...
    const subjectOption = subjectOptions.find(
      o => o.value === formData.subject,
    );
    const subjectValue =
      subjectOptions && subjectOptions.length > 1 && subjectOption
        ? subjectOption.label
    ...
    if (
      type === TicketType.EstablishedPatient ||
    ...
    const ticketData: Ticket = {
      ...
      assigneeFullName:
        formData.assignee !== ''
          ? users.find(a => a.value === formData.assignee)?.label
          : undefined,
  ...
  useEffect(() => {
    ...
    async function getPatientById() {
      ...
      try {
        const patient: Patient = await getPatientByIdWithQuery(
          Number(patientId),
        );
        if (!patient) {
          throw new Error();
        ...
        setError('patientId', {
          type: 'notFound',
          message: t('ticket_new.patient_id_not_found'),
        });
      } finally {
  ...
  const { refetch: patientCasesRefetch, isFetching: isFetchingPatientCases } =
    useQuery(
      [GetPatientCases, currentPatient?.patientId],
      () =>
        getPatientCases({
          patientId: currentPatient?.patientId!,
          departmentId: currentPatient?.currentDepartmentId!,
          limit: 50,
        }),
      {
        enabled: !!currentPatient && patientCaseOptions.length > 0,
  ...
  const validatePatientCaseId = async () => {
    ...
    try {
      if (!!patientId) {
        const patientCase = await getPatientCaseDocument(
          Number(patientId),
          Number(patientCaseId),
        );
        if (!patientCase) {
          throw new Error();
        }
      } else {
        const patientActionNotes = await getPatientActionNotes(
          Number(patientCaseId),
        );
      ...
      setError('patientCaseNumber', {
        type: 'notFound',
        message: t('ticket_new.patient_case_id_not_found'),
      });
    } finally {
  ...
  const getTicketLookupValuesOptionsByTicketType = (
    data: any[] | undefined,
  ) => {
    if (data) {
      ...
      const filtered = data.filter(
        v => v.parentValue === ticketType.toString(),
      );
  ...
  const subjectOptions: Option[] = getTicketLookupValuesOptionsByTicketType(
    ticketLookupValuesSubject,
  );
  const reasonOptions: Option[] = getTicketLookupValuesOptionsByTicketType(
  ...
  if (
    ...
    isLookupValuesLoading ||
    isTicketEnumValuesLoading
  ) {
  ...
  if (
    ...
    return (
      <div data-test-id='ticket-new-load-failed'>
        {t('ticket_new.load_failed')}
      </div>
    );
  }
  ...
  const handlePatientCaseFocus = () => {
    if (!currentPatient || patientCaseOptions.length > 0) {
      return;
    }
  ...
  return (
    <div className='flex flex-col w-full pb-5 mx-6 mt-5 overflow-y-auto'>
      <div className='h-18'>
      ...
      <div className='pt-7'>
          ...
          <div className='grid grid-cols-3 gap-x-8'>
            ...
            <div className='flex'>
              <ControlledDateInput
                ...
                onValidationError={() =>
                  setError('dueDate', { message: 'Validation error' })
                }
                isSmallSize={true}
              />
              <ControlledTimeInput
              ...
              {shouldDisplayField('patientId') && (
                <ControlledInput
                  ...
                  onBlur={({ target }: ChangeEvent<HTMLInputElement>) =>
                    onPatientIdChanged(target.value)
                  }
                />
              )}
            </div>
            <div>
              {shouldDisplayField('patientCaseNumber') && !!currentPatient && (
                <ControlledSelect
                  control={control}
                  name='patientCaseNumber'
                  label='ticket_new.patient_case_number'
                  options={patientCaseOptions}
                  allowClear={true}
                  isLoading={isFetchingPatientCases}
                  onFocus={handlePatientCaseFocus}
                  onTextChange={onPatientCaseIdChanged}
                  onSelect={option =>
                    onPatientCaseIdChanged(option?.value ?? '')
                  }
                />
                // <ControlledInput
                //   name='patientCaseNumber'
                ...
                //     onPatientCaseIdChanged(target.value)
                //   }
            ...
            {shouldDisplayField('patientId') && patientName && (
              <>
                <div className='flex flex-row items-center w-full h-12 px-6 mb-6 rounded-md new-ticket-patient-background'>
                  <div className='whitespace-pre body2-medium'>{`${t(
                    'ticket_new.patient_label',
                  )} `}</div>
            ...
            <div>
              {
                <div hidden={!shouldDisplayField('contactId')}>
                  <Controller
                    name='contactId'
                    control={control}
                    defaultValue=''
                    render={props => (
                      <Select
                        ...
                        suggestionsPlaceholder={t(
                          'ticket_new.suggestion_placeholder',
                        )}
                        onTextChange={(value: string) =>
                          setContactSearchTerm(value || '')
                        }
                        onSelect={option =>
                          onContactSelectChanged(props, option)
                        }
                      />
                    )}
                  />
            ...
            <div className='pt-8'>
              <Button
                data-test-id='ticket-new-cancel-button'
                type={'button'}
            ...
            <div className='flex justify-end pt-8'>
              <Button
                isLoading={
                  ...
                  isPatientCaseIdLoading ||
                  isPatientIdLoading
                }
                disabled={!isValid || !stateError}
                data-test-id='ticket-new-create-button'
                type={'submit'}
        ...
        <Confirmation
          title={t('common.confirm_close')}
          okButtonLabel={t('common.yes')}
          isOpen={closingPromptOpen}
          onOk={onCloseConfirm}
          onCancel={() => setClosingPromptOpen(false)}
          onClose={() => setClosingPromptOpen(false)}
