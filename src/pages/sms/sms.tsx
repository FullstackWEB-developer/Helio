...
import {
  ...
  markRead,
  sendMessage,
} from './services/ticket-messages.service';
import {
  DATE_INPUT_LONG_FORMAT,
...
const Sms = () => {
  ...
  const [selectedTicketSummary, setSelectedTicketSummary] =
    useState<TicketMessageSummary>();
  const [messages, setMessages] = useState<TicketMessage[]>([]);
  const [isNewSmsChat, setIsNewSmsChat] = useState(false);
  const [filterParam, setFilterParam] = useState<SmsFilterParamModel>({
    ...DEFAULT_FILTER_VALUE,
    assignedTo: id,
  });
  ...
  const {
    isLoading: isLoadingContactNames,
    isFetching: isFetchingContactNames,
  } = useQuery(
    [GetContactsNames, contactIds],
    () => getContactsNames(contactIds),
    {
      ...
      onSuccess: data => {
        ...
        data.forEach(element => {
          let index = copyOfPageResult.findIndex(
            x => x.contactId === element.id,
          );
          let sms = Object.assign(
            {},
            pageResult.find(x => x.contactId === element.id),
          );
  ...
  const {
    ...
    hasNextPage,
    isFetchingNextPage,
    refetch: refetchTicketSummaries,
  } = useInfiniteQuery(
    [QueryTicketMessageSummaryInfinite, queryParams],
  ...
  const { refetch: fetchTicket, isFetching: isTicketFetching } = useQuery(
    ['Query', ticketId],
  ...
  const { refetch: ticketSummaryRefetch, isFetching: isTicketSummaryFetching } =
    useQuery(
      [QueryTicketMessageSummaryByTicketId, ticketId],
  ...
  useEffect(() => {
    setSmsQueryType(
      isDefaultTeamView ? SmsQueryType.MyTeam : SmsQueryType.MySms,
    );
  ...
  const {
    ...
    [QueryTicketMessagesInfinite],
    ({ pageParam = 1 }) =>
      getMessages(selectedTicketSummary?.ticketId || '', ChannelTypes.SMS, {
        ...DEFAULT_MESSAGE_QUERY_PARAMS,
        page: pageParam,
      }),
    {
  ...
  const removeUnreadCount = (summary: TicketMessageSummary) => {
    if (summary.unreadCount > 0) {
      markReadMutation.mutate(
        { ticketId: summary.ticketId, channel: ChannelTypes.SMS },
        {
          onSuccess: () => {
  ...
  useQuery([GetTicketMessage, newMessageId], () => getMessage(newMessageId), {
    ...
    onSuccess: result => {
      const isTicketSummarySelected =
        summaryMessages &&
        selectedTicketSummary?.ticketId === result.ticketId &&
        result.channel === ChannelTypes.SMS;
      if (isTicketSummarySelected && !isSMSSentFromChatPage) {
        pushMessage(result);
      }
      if (
        !!botContext &&
        !isSMSSentFromChatPage &&
        botContext?.ticket?.id === selectedTicketSummary?.ticketId
      ) {
        pushMessage(result);
      }
  ...
  const markReadMutation = useMutation(
    ({ ticketId, channel }: { ticketId: string; channel: ChannelTypes }) =>
      markRead(ticketId, channel, TicketMessagesDirection.Incoming),
  );

  const receiveSMS = useCallback(
    (data: SmsNotificationData) => {
      const messageIndex = summaryMessages.findIndex(
        p => p.ticketId === data.ticketId,
      );
      if (messageIndex < 0) {
        refetchTicketSummaries().then();
      }
  ...
  const onSendMessage = (toAddress: string, text: string) => {
    ...
    if (
      !selectedTicketSummary.assignedTo ||
      selectedTicketSummary.assignedTo !== id
    ) {
      ...
      <div className='flex flex-col pt-6 border-r sms-sidebar'>
        <div className='pb-2 pl-5 border-b'>
          <DropdownLabel
            items={[
              {
                label: 'sms.query_type.my_sms',
                value: SmsQueryType.MySms,
              },
              {
                label: 'sms.query_type.team_sms',
                value: SmsQueryType.MyTeam,
              },
            ]}
            value={smsQueryType}
          ...
          <div className='flex-none border-b'>
            <div className='flex flex-row justify-between pt-4 pb-4 pl-5 pr-4'>
              <div className='subtitle2'>
                {getTimePeriodLabel(filterParam.timePeriod)}
              </div>
