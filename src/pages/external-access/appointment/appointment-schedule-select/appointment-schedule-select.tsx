...
const AppointmentScheduleSelect = () => {
  ...
  const [departmentLatLng, setDepartmentLatLng] = useState<
    AppointmentDepartmentModel[]
  >([]);
  const [isFilterOpen, setFilterOpen] = useState(false);
  const [selectedDate, setSelectedDate] = useState<Date>(
    appointmentSlotRequest?.startDate ?? dayjs().utc().local().toDate(),
  );
  ...
  useEffect(() => {
    if (isFilterOpen) {
      slotRequest.timeOfDays?.forEach(p => {
        setValue(`time_${AppointmentSlotTimeOfDay[p].toLowerCase()}`, {
          value: p.toString(),
          checked: true,
        });
      });
  ...
  const { isLoading: isAppointmentSlotsLoading, isFetching } = useQuery<
    AppointmentSlot[],
    AxiosError
  >(
    [GetAppointmentSlots, slotRequest],
    () => getAppointmentSlots({ ...slotRequest }, false),
    {
      ...
      onSuccess: (data: AppointmentSlot[]) => {
        ...
        const latLong = locations
          .filter(p => data.findIndex(d => p.id === d.departmentId) > -1)
          .map(p => {
            return {
        ...
        const slotLocationMap = (slot: AppointmentSlot) => ({
          ...slot,
          departmentName: locations.find(
            location => location.id === slot.departmentId,
          )?.name,
        });
        ...
        const sorted = data
          ...
          .sort(
            (a, b) =>
              dayjs.utc(a.date).toDate().getTime() -
              dayjs.utc(b.date).toDate().getTime(),
          );
  ...
  const changeDate = (
    newStartDate: Date,
    isUpdateDefaultDate: boolean = true,
  ) => {
    ...
    setSlotRequest({
      ...
      endDate: dayjs(newStartDate).add(7, 'day').toDate(),
    });
  ...
  const changeProvider = (event?: Option) => {
    ...
    setSlotRequest({
      ...
      endDate: dayjs(defaultDate).add(7, 'day').toDate(),
      providerId: !!event.value ? [Number(event.value)] : undefined,
    });
    setDisplayMaxTryError(false);
  };
  ...
  const changeLocation = (event?: Option) => {
    ...
    setSlotRequest({
      ...slotRequest,
      departmentId: !!event.value ? Number(event.value) : undefined,
    });
    setDisplayMaxTryError(false);
  };

  const nextPage = (isMobile = false) => {
    let nextStartDate = dayjs(selectedDate)
      .startOf('isoWeek')
      .utc()
      .add(7, 'day')
      .toDate();
  ...
  const previousPage = (isMobile = false) => {
    let prevStartDate = dayjs(selectedDate)
      .startOf('isoWeek')
      .utc()
      .add(-7, 'day')
      .toDate();
  ...
  const onTimeOfDayChanged = (event: CheckboxCheckEvent) => {
    ...
    if (event.checked) {
      ...
      slotRequestTimeOfDayCopy.splice(
        slotRequestTimeOfDayCopy.findIndex(p => p === timeOfDay),
        1,
      );
      setSlotRequest({ ...slotRequest, timeOfDays: slotRequestTimeOfDayCopy });
      setCurrentFilterCount(count => count - 1);
    ...
    <div className='flex flex-row appointment-schedule-select without-default-padding without-default-padding-right without-default-padding-bottom'>
      <div className='flex-1 pt-16 mr-14 pb-36'>
        <div className='flex items-center 2xl:whitespace-pre 2xl:h-12 2xl:mt-3 2xl:mb-9'>
          <h4>{t('external_access.appointments.select_your_appointment')}</h4>
        </div>
        ...
        <div className='flex flex-col'>
          ...
          <div>
            {(isAppointmentSlotsLoading || isFetching) && <Spinner />}
            {!isAppointmentSlotsLoading &&
              ...
              displayMaxTryError && (
                <div>
                  <div>
                    {t(
                      'external_access.appointments.no_appointment_found_in_max_tries',
                    )}
                  </div>
                  <div>
                    {t(
                      'external_access.appointments.no_appointment_found_in_max_tries_2',
                    )}
                  </div>
                </div>
              )}
                            ...
                            getWorkDates().map(date => (
                              <div
                                className={classnames(
                                  'flex justify-center reschedule-slot subtitle2 w-56 md:w-80 mx-2',
                                  {
                                    'slot-date-selected body2':
                                      isBordered(date),
                                  },
                                )}
                              >
                              ...
                              <>
                                {mappedSlots.get(
                                  ...
                                  <div data-test-id='external-access-appointments-no-day-slots'>
                                    {t(
                                      'external_access.appointments.no_day_slots_found',
                                    )}
                                  </div>
                                )}
                              </>
          ...
          <div className='pt-5 pb-12 h-full'>
            <div>
              <span className='subtitle2'>
                {t('external_access.schedule_appointment.time_of_day.title')}
              </span>
              <div className='mt-5'>
                <ControlledCheckbox
                  control={control}
            ...
            <div className='mt-6'>
              <span className='subtitle2'>
                {t('external_access.schedule_appointment.gender.title')}
              </span>
