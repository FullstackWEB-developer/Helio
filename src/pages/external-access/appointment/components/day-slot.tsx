...
const MAX_SLOTS = 8;
const DaySlots: FC<DaySlotsProps> = ({ column, hideShowMore, showAllSlot, ...props }) => {
  ...
  const ShowMore = () => {
    return (
      <div className='pb-4 cursor-pointer' onClick={() => setDisplayMore(!displayMore)}>
        <div className='flex flex-row flex-col items-center justify-center w-56 h-12 px-4 day-slot reschedule-slot md:w-80'>
          <div className='body2'>{displayMore ? t('common.show_less') : t('common.show_more')}</div>
        </div>
      </div>
    );
  };
  ...
  const Slot = ({ value }: { value: AppointmentSlot }) => {
    return (
      <div className='pb-4 cursor-pointer md:w-full'>
        <div
          className='flex flex-row flex-col items-center justify-center w-56 h-12 px-4 day-slot reschedule-slot md:w-80 lg:w-32'
          onClick={() => selectSlot(value)}
        >
          <div className='body2'>{dayjs(value.startTime, 'HH:mm').format('h:mm A')}</div>
          <ElipsisTooltipTextbox value={value.departmentName ?? ''} classNames='body3-small truncate w-24 text-center' asSpan />
        </div>
      </div>
    );
  };
  ...
  if (!column || !column.slots) {
    return (
      <div className='flex justify-center w-56 p-2 mx-2 md:w-80 reschedule-slot'>
      ...
      {column && (
        <div className='grid grid-flow-row p-2 auto-rows-max md:auto-rows-min'>
          {column.slots
            ...
            .map((slot: AppointmentSlot, index: number) => (
