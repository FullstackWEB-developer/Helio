...
const RegistrationReviewStep = ({goBack}: RegistrationReviewStepProps) => {
    ...
    const displayAddressField = () => {
        ...
        const apt = patientData?.patient?.address2 ? `${t(patientData?.patient?.address2.includes('#') ? 'patient.summary.apt_number_no_#' : 'patient.summary.unit')} ${patientData?.patient?.address2}`: '';
