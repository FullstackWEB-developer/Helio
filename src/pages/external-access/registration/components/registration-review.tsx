...
import { selectRegisteredPatient, selectRegisteredPatientInsurance } from '@pages/external-access/registration/store/registration.selectors';
...
import { cleanRegistrationState } from '../store/registration.slice';
...
const RegistrationReviewStep = ({ goBack }: RegistrationReviewStepProps) => {
  ...
  const displayAddressField = () => {
    ...
    const apt = patientData?.patient?.address2
      ? `${t(patientData?.patient?.address2.includes('#') ? 'patient.summary.apt_number_no_#' : 'patient.summary.unit')} ${patientData?.patient?.address2}`
      : '';
  ...
  return (
    <div className='flex flex-col'>
      <div className='body2'>
        <span className='review-label'>{`${t('external_access.first_name')}: `}</span>
        {patientData?.patient?.firstName}
      </div>
      <div className='body2'>
        <span className='review-label'>{`${t('external_access.last_name')}: `}</span>
        {patientData?.patient?.lastName}
      </div>
      <div className='body2'>
        <span className='review-label'>{`${t('external_access.registration.dob')}: `}</span>
        {utils.formatDate(patientData?.patient?.dateOfBirth)}
      </div>
      <div className='body2'>
        <span className='review-label'>{`${t('external_access.registration.mobile_phone')}: `}</span>
        {utils.formatPhone(patientData?.patient?.mobilePhone)}
      </div>
      {patientData.patient.email ? (
        <div className='body2'>
          <span className='review-label'>{`${t('external_access.registration.email')}: `}</span>
          {patientData?.patient?.email}
        </div>
      ) : null}
      <div className='body2'>
        <span className='review-label'>{`${t('external_access.registration.address')}: `}</span>
        {displayAddressField()}
      </div>
      <div className='body2'>
        <span className='review-label'>{`${t('external_access.registration.sex')}: `}</span>
        {patientData?.patient?.sex}
      </div>
      <div className='pt-10' />
      <div className='body2'>
        <span className='review-label'>{`${t('external_access.registration.text_consent_short')}: `}</span>
        {displayConsents('text')}
      </div>
      <div className='body2'>
        <span className='review-label'>{`${t('external_access.registration.call_consent_short')}: `}</span>
        {displayConsents('call')}
      </div>
      <div className='body2'>
        <span className='review-label'>{`${t('external_access.registration.preferred_communication_short')}: `}</span>
        {t(`${displayPreferredCommunication(patientData?.patient?.contactPreference)}`)}
      </div>
      <div className='body2'>
        <span className='review-label'>{`${t('external_access.registration.referral_source')}: `}</span>
        {t(`${displayReferralSource(patientData?.patient?.referralSourceId)}`)}
      </div>
      <div className='pt-10' />
      {insuranceData?.insuranceOption?.value === INSURANCE_PLAN ? (
        <>
          <div className='body2'>
            <span className='review-label'>{`${t('external_access.registration.insurance_type')}: `}</span>
            {insuranceData?.insuranceType}
          </div>
          <div className='body2'>
            <span className='review-label'>{`${t('external_access.registration.insurance_name')}: `}</span>
            {insuranceData?.insuranceName}
          </div>
          <div className='body2'>
            <span className='review-label'>{`${t('external_access.registration.policy_holder_name')}: `}</span>
            {insuranceData?.policyHolderName}
          </div>
          <div className='body2'>
            <span className='review-label'>{`${t('external_access.registration.policy_holder_dob')}: `}</span>
            {utils.formatDate(insuranceData?.policyHolderDob)}
          </div>
          <div className='body2'>
            <span className='review-label'>{`${t('external_access.registration.policy_holder_relationship')}: `}</span>
            {insuranceData?.insuranceRelation}
          </div>
          <div className='body2'>
            <span className='review-label'>{`${t('external_access.registration.insurance_member_id')}: `}</span>
            {insuranceData?.insuranceMemberId}
          </div>
          {insuranceData?.groupNumber ? (
            <div className='body2'>
              <span className='review-label'>{`${t('external_access.registration.group_number')}: `}</span>
              {insuranceData?.groupNumber}
            </div>
          ) : null}
        </>
        ...
        <div className='body2'>
          <span className='review-label'>{`${t('external_access.registration.insurance_info')}: `}</span>
          {t(`${insuranceData?.insuranceOption?.label}`)}
        </div>
      )}
      {!successfullyCompletedRegistration && (
        <div className='flex pt-6'>
          ...
          <Button type='submit' label='common.submit' onClick={() => confirm()} isLoading={updatePatientMutation.isLoading} className='w-36' />
        </div>
      )}
    </div>
  );
