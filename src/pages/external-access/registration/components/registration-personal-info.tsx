...
const PersonalInformationRegistrationStep = ({
  onPatientUpsert,
}: PersonalInformationRegistrationStepProps) => {
  ...
  const displayDobErrorMessage = () => {
    if (errors.dob?.type) {
      switch (errors.dob?.type) {
        case "required":
          return t("external_access.invalid_dob", {
            format: utils.getBrowserDatePattern(),
          });
        case "max":
  ...
  const genderOptions: Option[] = [
    ...
    {
      value: "NotDisclose",
      label: "external_access.registration.gender_not_disclose",
    },
    {
      value: "Unknown",
      label: "external_access.registration.gender_unknown",
    },
  ];

  const referralSourceLookupValues = useSelector((state) =>
    selectLookupValues(state, "ReferralSource")
  );
  const referralSourceItems: Option[] = referralSourceLookupValues.map(
    (sourceItem) => {
      return {
  ...
  if (utils.isValidDobByAthenaMaxAgeConstraint(dobValue) && !errors.dob) {
    setError("dob", {
      type: "max",
      message: "external_access.registration.max_age",
    });
  }
  ...
  const onSubmit = (formData: any) => {
    const departmentId =
      Number(utils.getAppParameter("DefaultDepartmentId")) ?? 3;
    const patient: CreatePatientModel = {
      ...
      sex: utils.isString(formData.gender)
        ? formData.gender.charAt(0)
        : formData.gender.value.charAt(0),
      zip: formData.zip,
    ...
    if (!!patientData?.registrationSessionKey) {
      createPatientRequest.registrationSessionKey =
        patientData.registrationSessionKey;
    }

    createPatientMutation.mutate(createPatientRequest, {
      onSuccess: (data) => {
        createPatientRequest.registrationSessionKey =
          data.registrationSessionKey;
        ...
        dispatch(
          addSnackbarMessage({
            ...
            message:
              ...
              "external_access.registration.patient_creation_failure",
          })
        );
      },
    });
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col w-full">
      ...
      <div className="flex flex-col md:flex-row md:gap-8">
        ...
        <ControlledSelect
          required={true}
          name="gender"
          label="external_access.registration.gender"
          className="md:w-1/2"
      ...
      <div className="flex flex-col">
        <div className="body2">
          {t("external_access.registration.referral_source")}
        </div>
        <ControlledSelect
          required={true}
          name="referralSource"
