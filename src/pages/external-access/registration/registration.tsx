...
import { setCurrentRegistrationStep } from '@pages/external-access/registration/store/registration.slice';
import { selectCurrentRegistrationStep } from './store/registration.selectors';

const Registration = () => {
  const dispatch = useDispatch();
  const { t } = useTranslation();
  const currentStep = useSelector(selectCurrentRegistrationStep);
  ...
  const goStepForward = () => {
    if (currentStep < RegistrationStep.Review) {
      dispatch(setCurrentRegistrationStep(currentStep + 1));
    }
  };
  ...
  return (
    <div>
      ...
      <div className='w-11/12 whitespace-pre-line'>{displayStepDescription()}</div>
