...
const CompleteReschedulingForm = ({selectedAppointmentId} : CompleteReschedulingFormProps) => {
    ...
    const getSlotOptions = (): Option[]  => {
        if (openSlots && openSlots.length >0) {
            let options = openSlots.map(slot => {
                const date = new Date(slot.date);
                const dateStr = ` ${utils.dateToString(date, "MM/dd/yyyy")} at ${slot.startTime}`;
                let label = `${dateStr}`;
                return {
    ...
    const reschedule = (data: any) => {
        dispatch(rescheduleAppointment(selectedAppointmentId as string, parseInt(data.newAppointmentId), verifiedPatient.patientId));
    }
