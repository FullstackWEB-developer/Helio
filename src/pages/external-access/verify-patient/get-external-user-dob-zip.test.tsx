...
import GetExternalUserDobZip from './get-external-user-dob-zip';
...
describe("Feedback tests", () => {
    ...
    it("renders GetExternalUserDobZip not-verified", async () => {
        let response = {
            data: {
                doesExists: true,
                email: "string",
        ...
        await act(async () => {
            const {asFragment, getByTestId} = render(<TestWrapper mockState={mockState}>
                <GetExternalUserDobZip/>          
            </TestWrapper>);
            fragment = asFragment;
            await waitFor(() => {
                fireEvent.input(getByTestId('zip'), {target: {value: 'checked'}});
            });
            await waitFor(() => {
