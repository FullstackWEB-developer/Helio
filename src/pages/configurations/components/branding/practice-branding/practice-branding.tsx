...
import { getPracticeEmailTemplatePreviewFromBranding } from '@shared/services/notifications.service';
...
const PracticeBrandingEdit = () => {
  ...
  return (
    <div className='practice-branding overflow-auto h-full pr-4'>
      {' '}
      {isFetching ? (
        <Spinner fullScreen />
      ) : (
        <form onSubmit={handleSubmit(onSubmit)} className='px-6 py-6 flex flex-1 flex-col overflow-y-auto body2'>
          ...
          <div className='color-picker-grid mb-6'>
            ...
            <div className='ml-20'>
              <ControlledColorPicker control={control} name={'primaryColor'} onChangeColor={(key: string, color: string) => updateCurrentColor(key, color)} />
            </div>
            <div className='flex flex-col'>
            ...
            <div className='ml-20'>
              <ControlledColorPicker control={control} name={'hoverColor'} onChangeColor={(key: string, color: string) => updateCurrentColor(key, color)} />
            </div>
            <div className='flex flex-col'>
            ...
            <div className='ml-20'>
              <ControlledColorPicker control={control} name={'focusedColor'} onChangeColor={(key: string, color: string) => updateCurrentColor(key, color)} />
            </div>
            <div className='flex flex-col'>
            ...
            <div className='ml-20'>
              <ControlledColorPicker control={control} name={'secondaryColor'} onChangeColor={(key: string, color: string) => updateCurrentColor(key, color)} />
            </div>
            <div className='flex flex-col'>
            ...
            <div className='ml-20'>
              <ControlledColorPicker control={control} name={'tertiaryColor'} onChangeColor={(key: string, color: string) => updateCurrentColor(key, color)} />
            </div>
          </div>

          <h5 className='practice-branding-header-bottom-margin subtitle2'> {t('configuration.practice_branding.preview_title')} </h5>
          <h5
            className='practice-branding-subheader-bottom-margin body2-primary cursor-pointer w-max hover:underline'
            onClick={() =>
              window.open(
                `${ConfigurationsPath}/web-form-preview?isPreview=true&logoPath=${logoPath}&primaryColor=${colors?.primaryColor.replaceAll(
                  '#',
                  '',
                )}&secondaryColor=${colors?.secondaryColor.replaceAll('#', '')}&hoverColor=${colors?.hoverColor.replaceAll(
                  '#',
                  '',
                )}&focusedColor=${colors?.focusedColor.replaceAll('#', '')}&tertiaryColor=${colors?.tertiaryColor.replaceAll('#', '')}`,
                '_blank',
                'noopener,noreferrer',
              )
            }
          >
            {t('configuration.practice_branding.web_form_preview')}
          </h5>
          <div className='body2-primary practice-branding-subheader2-bottom-margin cursor-pointer w-max hover:underline' onClick={() => previewEmailTemplate()}>
